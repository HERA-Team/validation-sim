...
from redshifted_gaussian_fields import ParameterizedGaussianPowerSpectrum
from redshifted_gaussian_fields import con as zconst


def get_pspec_from_covariance_file(
    full_file_path: str | Path,
) -> ParameterizedGaussianPowerSpectrum:
...
def get_planck15_cosmo():
    ...
    return hp.conversions.Cosmo_Conversions(
        Om_L=0.6911, Om_b=0.02230 / h**2, Om_c=0.1188 / h**2, H0=H0
    )
...
def get_zs(
    cosmo: hp.conversions.Cosmo_Conversions, uvp: hp.UVPSpec, spw: int
) -> np.ndarray:
...
def tapered_power_spectrum(
    ...
    Pk = true_power_spectrum(
        cosmo, pgps, k, z, rescale_idx=rescale_idx, littleh=littleh
    )
