...
def compute_grf_covariance(test_mode: bool = False, ell_max: int = 1250):
    ...
    normalization_amplitude = 1.0

    Pspec = generator.ParameterizedGaussianPowerSpectrum(
        a,
        k0,
        renormalization=(normalization_point, normalization_amplitude),
    ...
    Np = 15
    eps = 1e-15
    gcfg = generator.GaussianCosmologicalFieldGenerator(
        cosmo, Pspec, nu_axis, del_nu, ell_axis, Np=Np, eps=eps
    )
